<a class="btn btn-danger" id="bulk_delete_btn"><i class="hymer-trash"></i> <span>{{ __('hymer::generic.bulk_delete') }}</span></a>
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title d-flex align-items-center">
                    ...
                    {{ __('hymer::generic.are_you_sure_delete') }}
                    <span id="bulk_delete_count"></span>
                    <span id="bulk_delete_display_name"></span>?
                </h5>
                <a role="button" class="btn modal-close" data-bs-dismiss="modal" aria-label="Close"><span
            ...
            <div class="modal-footer">
                ...
                <form action="{{ route('hymer.'.$dataType->slug.'.index') }}/0" id="bulk_delete_form" method="POST">
                    {{ method_field("DELETE") }}
                    {{ csrf_field() }}
                    <input type="hidden" name="ids" id="bulk_delete_input" value="">
                    <input type="submit" class="btn btn-danger float-end delete-confirm"
                             value="{{ __('hymer::generic.bulk_delete_confirm') }} {{ strtolower($dataType->getTranslatedAttribute('display_name_plural')) }}">
                </form>
            </div>
        </div><!-- /.modal-content -->
    </div><!-- /.modal-dialog -->
...
window.onload = function () {
    ...
    $bulkDeleteBtn.click(function () {
        ...
        if (count) {
            ...
            // No row selected
            toastr.warning('{{ __('hymer::generic.bulk_delete_nothing') }}');
        }
    });
}
</script>
