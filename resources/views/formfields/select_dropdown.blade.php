@if(isset($options->relationship))
    ...
    @if( !method_exists( $dataType->model_name, Str::camel($row->field) ) )
        <p class="label label-warning"><i class="hymer-warning"></i> {{ __('hymer::form.field_select_dd_relationship', ['method' => Str::camel($row->field).'()', 'class' => $dataType->model_name]) }}</p>
    @endif
    ...
    @if( method_exists( $dataType->model_name, Str::camel($row->field) ) )
        ...
        <select class="form-control select2" name="{{ $row->field }}">
            ...
            @if(isset($options->options))
                <optgroup label="{{ __('hymer::generic.custom') }}">
            ...
            <optgroup label="{{ __('hymer::database.relationship.relationship') }}">
