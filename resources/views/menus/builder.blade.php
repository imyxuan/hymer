...
@section('page_title', __('hymer::generic.menu_builder'))

@section('page_header')
    <h1 class="page-title d-flex align-items-center">
        <i class="hymer-list"></i>
        <span>{{ __('hymer::generic.menu_builder') }} ({{ $menu->name }})</span>
        <div class="ms-3">
            <div class="btn btn-success add_item">
                <i class="hymer-plus"></i> {{ __('hymer::menu_builder.new_menu_item') }}
            </div>
        </div>
    </h1>
...
@section('content')
            ...
            <div class="col-md-12">
                <div class="panel panel-bordered">
                    <div class="panel-heading">
                        <p class="panel-title" style="color:#777">{{ __('hymer::menu_builder.drag_drop_info') }}</p>
                    </div>
            ...
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">
                        ...
                        {{ __('hymer::menu_builder.delete_item_question') }}
                    </h5>
                ...
                <div class="modal-footer">
                    <button type="button" class="btn" data-bs-dismiss="modal">{{ __('hymer::generic.cancel') }}</button>
                    <form action="{{ route('hymer.menus.item.destroy', ['menu' => $menu->id, 'id' => '__id']) }}"
                          id="delete_form"
                          method="POST">
                        {{ method_field("DELETE") }}
                        {{ csrf_field() }}
                        <input type="submit" class="btn btn-danger float-end delete-confirm"
                               value="{{ __('hymer::menu_builder.delete_item_confirm') }}">
                    </form>
                </div>
            </div><!-- /.modal-content -->
        </div><!-- /.modal-dialog -->
    ...
    <div class="modal modal-info fade" tabindex="-1" id="menu_item_modal" role="dialog">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 id="m_hd_add" class="modal-title hidden">
                        <i class="hymer-plus"></i> {{ __('hymer::menu_builder.create_new_item') }}
                    </h5>
                    <h5 id="m_hd_edit" class="modal-title hidden">
                        <i class="hymer-edit"></i> {{ __('hymer::menu_builder.edit_item') }}
                    </h5>
                    <button type="button" class="btn modal-close" data-bs-dismiss="modal" aria-label="{{ __('hymer::generic.close') }}"><span
                            aria-hidden="true">&times;</span></button>
                </div>
                <form action="" id="m_form" method="POST"
                      data-action-add="{{ route('hymer.menus.item.add', ['menu' => $menu->id]) }}"
                      data-action-update="{{ route('hymer.menus.item.update', ['menu' => $menu->id]) }}">
                    ...
                    <div class="modal-body">
                        <div>
                            @include('hymer::multilingual.language-selector')
                            <label for="name">{{ __('hymer::menu_builder.item_title') }}</label>
                            @include('hymer::multilingual.input-hidden', ['_field_name' => 'title', '_field_trans' => ''])
                            <input type="text" class="form-control" id="m_title" name="title" placeholder="{{ __('hymer::generic.title') }}"><br>
                        </div>
                        <label for="type">{{ __('hymer::menu_builder.link_type') }}</label>
                        <select id="m_link_type" class="form-control" name="type">
                            <option value="url" selected="selected">{{ __('hymer::menu_builder.static_url') }}</option>
                            <option value="route">{{ __('hymer::menu_builder.dynamic_route') }}</option>
                        </select><br>
                        <div id="m_url_type">
                            <label for="url">{{ __('hymer::menu_builder.url') }}</label>
                            <input type="text" class="form-control" id="m_url" name="url" placeholder="{{ __('hymer::generic.url') }}"><br>
                        </div>
                        <div id="m_route_type">
                            <label for="route">{{ __('hymer::menu_builder.item_route') }}</label>
                            <input type="text" class="form-control" id="m_route" name="route" placeholder="{{ __('hymer::generic.route') }}"><br>
                            <label for="parameters">{{ __('hymer::menu_builder.route_parameter') }}</label>
                        ...
                        <label for="icon_class">{{ __('hymer::menu_builder.icon_class') }} <a
                                    href="{{ route('hymer.compass.index') }}#fonts"
                                    target="_blank">{!! __('hymer::menu_builder.icon_class2') !!}</label>
                        <input type="text" class="form-control" id="m_icon_class" name="icon_class"
                               placeholder="{{ __('hymer::menu_builder.icon_class_ph') }}"><br>
                        <label for="color">{{ __('hymer::menu_builder.color') }}</label>
                        <input type="color" class="form-control" id="m_color" name="color"
                               placeholder="{{ __('hymer::menu_builder.color_ph') }}"><br>
                        <label for="target">{{ __('hymer::menu_builder.open_in') }}</label>
                        <select id="m_target" class="form-control" name="target">
                            <option value="_self" selected="selected">{{ __('hymer::menu_builder.open_same') }}</option>
                            <option value="_blank">{{ __('hymer::menu_builder.open_new') }}</option>
                        </select>
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn" data-bs-dismiss="modal">
                            {{ __('hymer::generic.cancel') }}</button>
                        <input type="submit"
                               class="btn btn-primary delete-confirm__" value="{{ __('hymer::generic.update') }}">
                    </div>
                </form>
            </div><!-- /.modal-content -->
        </div><!-- /.modal-dialog -->
    ...
    <script>
        $(document).ready(function () {
            ...
            $('.add_item').click(function() {
                ...
                $m_form.find("input[type=submit]").val('{{ __('hymer::generic.add') }}');
            ...
            $('.item_actions').on('click', '.edit', function (e) {
                $m_form.find("input[type=submit]").val('{{ __('hymer::generic.update') }}');
            ...
            $('.item_actions').on('click', '.delete', function (e) {
                ...
                $('#delete_form')[0].action = '{{ route('hymer.menus.item.destroy', ['menu' => $menu->id, 'id' => '__id']) }}'.replace('__id', id);
            ...
            $('.dd').on('change', function (e) {
                $.post('{{ route('hymer.menus.order_item',['menu' => $menu->id]) }}', {
                    order: JSON.stringify($('.dd').nestable('serialize')),
                    _token: '{{ csrf_token() }}'
                }, function (data) {
                    toastr.success("{{ __('hymer::menu_builder.updated_order') }}");
                });
            });
        });
    </script>
