...
class CategoriesTableSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        if (!$dataType->exists) {
            $dataType->fill([
                'slug'                  => 'categories',
                'display_name_singular' => __('hymer::seeders.data_types.category.singular'),
                'display_name_plural'   => __('hymer::seeders.data_types.category.plural'),
                'icon'                  => 'hymer-categories',
                'model_name'            => 'PickOne\\Hymer\\Models\\Category',
                'controller'            => '',
        ...
        if (!$dataRow->exists) {
            $dataRow->fill([
                ...
                'display_name' => __('hymer::seeders.data_rows.parent'),
                'required'     => 0,
                'browse'       => 0,
                'read'         => 1,
                'edit'         => 1,
        ...
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'text',
                'display_name' => __('hymer::seeders.data_rows.order'),
                'required'     => 1,
                'browse'       => 1,
                'read'         => 1,
                'edit'         => 1,
        ...
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'text',
                'display_name' => __('hymer::seeders.data_rows.name'),
                'required'     => 1,
                'browse'       => 1,
                'read'         => 1,
                'edit'         => 1,
        ...
        $menuItem = MenuItem::firstOrNew([
            'menu_id' => $menu->id,
            'title'   => __('hymer::seeders.menu_items.categories'),
            'url'     => '',
