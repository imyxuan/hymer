...
Starting with Hymer 1.2 you can define a \(custom\) guard which is used throughout Hymer.  
To do so, just bind the name of your auth-guard to `HymerGuard`.  
...
and a model which extends Hymers user-model:
...
Next you have to tell Hymer to use your new guard.  
