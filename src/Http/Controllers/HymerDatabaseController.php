...
class HymerDatabaseController extends Controller
{
    public function index()
    {
        ...
        return Hymer::view('hymer::tools.database.index')->with(compact('dataTypes', 'tables'));
    }
    ...
    public function create()
    {
        ...
        return Hymer::view('hymer::tools.database.edit-add', compact('db'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if (isset($request->create_model) && $request->create_model == 'on') {
                ...
                Artisan::call('hymer:make:model', $params);
            ...
            return redirect()
               ...
               ->with($this->alertSuccess(__('hymer::database.success_create_table', ['table' => $table->name])));
        } catch (Exception $e) {
    ...
    public function edit($table)
    {
        ...
        if (!SchemaManager::tableExists($table)) {
            return redirect()
                ->route('hymer.database.index')
                ->with($this->alertError(__('hymer::database.edit_table_not_exist')));
        }
    ...
    protected function prepareDbManager($action, $table = '')
    {
        ...
        if ($action == 'update') {
            ...
            $db->formAction = route('hymer.database.update', $table);
        } else {
            ...
            $db->formAction = route('hymer.database.store');
        }
    ...
    public function show($table)
    {
        ...
        $model_name = Hymer::model('DataType')->where('name', $table)->pluck('model_name')->first();
        if (isset($model_name)) {
    ...
    public function destroy($table)
    {
        ...
        try {
            ...
            return redirect()
                ->route('hymer.database.index')
                ->with($this->alertSuccess(__('hymer::database.success_delete_table', ['table' => $table])));
        } catch (Exception $e) {
