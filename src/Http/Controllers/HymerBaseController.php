...
class HymerBaseController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (!empty($dataTypeContent->first())) {
            foreach (Hymer::actions() as $action) {
    ...
    public function show(Request $request, $id)
    {
        ...
        if (view()->exists("hymer::$slug.read")) {
            $view = "hymer::$slug.read";
        }

        return Hymer::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable', 'isSoftDeleted'));
    }
    ...
    public function edit(Request $request, $id)
    {
        ...
        $view = 'hymer::bread.edit-add';

        if (view()->exists("hymer::$slug.edit-add")) {
            $view = "hymer::$slug.edit-add";
        }

        return Hymer::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        if (auth()->user()->can('browse', app($dataType->model_name))) {
            $redirect = redirect()->route("hymer.{$dataType->slug}.index");
        } else {
    ...
    public function destroy(Request $request, $id)
    {
        ...
        $data = $affected
            ? [
                'message'    => __('hymer::generic.successfully_deleted')." {$displayName}",
                'alert-type' => 'success',
            ]
            : [
                'message'    => __('hymer::generic.error_deleting')." {$displayName}",
                'alert-type' => 'error',
            ];

        return redirect()->route("hymer.{$dataType->slug}.index")->with($data);
    }
    ...
    public function restore(Request $request, $id)
    {
        ...
        $data = $res
            ? [
                'message'    => __('hymer::generic.successfully_restored')." {$displayName}",
                'alert-type' => 'success',
            ]
            : [
                'message'    => __('hymer::generic.error_restoring')." {$displayName}",
                'alert-type' => 'error',
            ];
    ...
    public function remove_media(Request $request)
    {
        try {
            ...
            if (!isset($data->{$field})) {
                throw new Exception(__('hymer::generic.field_does_not_exist'), 400);
            }
            ...
            if (@json_decode($multi)) {
                ...
                if (is_null(@json_decode($data->{$field}))) {
                    throw new Exception(__('hymer::json.invalid'), 500);
                }
                ...
                if (is_null($key) || $key === false) {
                    throw new Exception(__('hymer::media.file_does_not_exist'), 400);
                }
            ...
            return response()->json([
                'data' => [
                    'status'  => 200,
                    'message' => __('hymer::media.file_removed'),
                ],
            ]);
        } catch (Exception $e) {
            $code = 500;
            $message = __('hymer::generic.internal_error');
    ...
    public function deleteBreadImages($data, $rows, $single_image = null)
    {
        ...
        foreach ($rows as $row) {
            ...
            foreach ($images_to_remove as $image) {
                ...
                if ($image != config('hymer.user.default_avatar') && (is_null($single_image) || $single_image == $image)) {
    ...
    public function order(Request $request)
    {
        ...
        if (empty($dataType->order_column) || empty($dataType->order_display_column)) {
            ...
            ->with([
                'message'    => __('hymer::bread.ordering_not_set'),
                'alert-type' => 'error',
            ]);
        }
        ...
        $dataRow = Hymer::model('DataRow')->whereDataTypeId($dataType->id)->whereField($display_column)->first();

        $view = 'hymer::bread.order';

        if (view()->exists("hymer::$slug.order")) {
            $view = "hymer::$slug.order";
        }
