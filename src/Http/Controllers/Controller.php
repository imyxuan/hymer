...
abstract class Controller extends BaseController
{
    ...
    public function insertUpdateData($request, $slug, $rows, $data)
    {
        ...
        if ($request->session()->has($slug.'_path') || $request->session()->has($slug.'_uuid')) {
            ...
            if ($old_path != $new_path &&
                !Storage::disk(config('hymer.storage.disk'))->exists($new_path) &&
                Storage::disk(config('hymer.storage.disk'))->exists($old_path)
                )
            {
                $request->session()->forget([$slug.'_path', $slug.'_uuid']);
                Storage::disk(config('hymer.storage.disk'))->move($old_path, $new_path);
                Storage::disk(config('hymer.storage.disk'))->deleteDirectory($folder_path);
            }
        }
    ...
    {
        if ($path && Storage::disk(config('hymer.storage.disk'))->exists($path)) {
            Storage::disk(config('hymer.storage.disk'))->delete($path);
