...
class HymerBreadController extends Controller
{
    public function index()
    {
        ...
        $dataTypes = Hymer::model('DataType')->select('id', 'name', 'slug')->get()->keyBy('name')->toArray();
        ...
        return Hymer::view('hymer::tools.bread.index')->with(compact('dataTypes', 'tables'));
    }
    ...
    public function create(Request $request, $table)
    {
        ...
        $dataType = Hymer::model('DataType')->whereName($table)->first();
        ...
        return Hymer::view('hymer::tools.bread.edit-add', $data);
    }

    private function prepopulateBreadInfo($table)
    {
        $displayName = Str::singular(implode(' ', explode('_', Str::title($table))));
        $modelNamespace = config('hymer.models.namespace', app()->getNamespace());
    ...
    public function store(Request $request)
    {
        ...
        try {
            $dataType = Hymer::model('DataType');
            $res = $dataType->updateDataType($request->all(), true);
            $data = $res
                ? $this->alertSuccess(__('hymer::bread.success_created_bread'))
                : $this->alertError(__('hymer::bread.error_creating_bread'));
            if ($res) {
                event(new BreadAdded($dataType, $data));
            }

            return redirect()->route('hymer.bread.index')->with($data);
        } catch (Exception $e) {
            return redirect()->route('hymer.bread.index')->with($this->alertException($e, 'Saving Failed'));
        }
    }
    ...
    public function edit($table)
    {
        ...
        $dataTypeRelationships = Hymer::model('DataRow')->where('data_type_id', '=', $dataType->id)->where('type', '=', 'relationship')->get();
        $scopes = [];
        if ($dataType->model_name != '') {
            $scopes = $this->getModelScopes($dataType->model_name);
        }

        return Hymer::view('hymer::tools.bread.edit-add', compact('dataType', 'fieldOptions', 'isModelTranslatable', 'tables', 'dataTypeRelationships', 'scopes'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            $res = $dataType->updateDataType($request->all(), true);
            $data = $res
                ? $this->alertSuccess(__('hymer::bread.success_update_bread', ['datatype' => $dataType->name]))
                : $this->alertError(__('hymer::bread.error_updating_bread'));
            if ($res) {
            ...
            return back()->with($this->alertException($e, __('hymer::generic.update_failed')));
        }
    }
    ...
    public function destroy($id)
    {
        ...
        $res = Hymer::model('DataType')->destroy($id);
        $data = $res
            ? $this->alertSuccess(__('hymer::bread.success_remove_bread', ['datatype' => $dataType->name]))
            : $this->alertError(__('hymer::bread.error_updating_bread'));
        if ($res) {
        ...
        if (!is_null($dataType)) {
            Hymer::model('Permission')->removeFrom($dataType->name);
        }
    ...
    public function addRelationship(Request $request)
    {
        ...
        if (!class_exists($request->relationship_model)) {
            return back()->with([
                'message' => 'Model Class ' . $request->relationship_model .
                    ' does not exist. Please create Model before creating relationship.',
                'alert-type' => 'error',
            ]);
        }
        ...
        try {
            ...
            $className = Hymer::modelClass('DataRow');
            ...
            $newRow->order = intval(Hymer::model('DataType')->find($request->data_type_id)->lastRow()->order) + 1;
    ...
    private function getRelationshipField($request)
    {
        ...
        $dataType = Hymer::model('DataType')->find($request->data_type_id);
        ...
        $existingRow = Hymer::model('DataRow')->where('field', '=', $relationshipField)->first();
        $index = 1;
    ...
    {
        Hymer::model('DataRow')->destroy($id);
