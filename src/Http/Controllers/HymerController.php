...
class HymerController extends Controller
{
    ...
    public function logout()
    {
        ...
        return redirect()->route('hymer.login');
    }
    ...
    public function upload(Request $request)
    {
        ...
        $dataType = Hymer::model('DataType')->where('slug', '=', $slug)->firstOrFail();
        ...
        while (Storage::disk(config('hymer.storage.disk'))->exists($path.$filename.'.'.$file->getClientOriginalExtension())) {
        ...
        if (in_array($ext, ['jpeg', 'jpg', 'png', 'gif'])) {
            ...
            if (Storage::disk(config('hymer.storage.disk'))->put($fullPath, (string) $image, 'public')) {
                $status = __('hymer::media.success_uploading');
                $fullFilename = $fullPath;
            } else {
                $status = __('hymer::media.error_uploading');
            }
        } else {
            $status = __('hymer::media.uploading_wrong_type');
        }

        // Return URL for TinyMCE
        return Hymer::image($fullFilename);
    }
