...
class Translator implements ArrayAccess, JsonSerializable
{
    ...
    public function __construct(Model $model)
    {
        ...
        $this->model = $model;
        $this->locale = config('hymer.multilingual.default', 'en');
        $attributes = [];
    ...
    public function save()
    {
        ...
        foreach ($attributes as $key => $attribute) {
            if ($attribute['exists']) {
                ...
                $translation = HymerFacade::model('Translation')->where('table_name', $this->model->getTable())
            ...
            if (is_null($translation)) {
                $translation = HymerFacade::model('Translation');
            }
    ...
    protected function translateAttributeToOriginal($attribute)
    {
        $this->attributes[$attribute] = [
            ...
            'locale'   => config('hymer.multilingual.default', 'en'),
            'exists'   => true,
            'modified' => false,
        ];
    ...
    public function deleteTranslation($key)
    {
        ...
        HymerFacade::model('Translation')->where('table_name', $this->model->getTable())
            ->where('column_name', $key)
            ->where('foreign_key', $this->model->getKey())
            ->where('locale', $locale)
            ->delete();
