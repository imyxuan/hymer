...
trait Translatable
{
    ...
    {
        return $this->hasMany(Hymer::model('Translation'), 'foreign_key', $this->getKeyName())
            ->where('table_name', $this->getTable())
            ->whereIn('locale', config('hymer.multilingual.locales', []));
    }
    ...
    public function getTranslationsOf($attribute, array $languages = null, $fallback = true)
    {
        if (is_null($languages)) {
            $languages = config('hymer.multilingual.locales', [config('hymer.multilingual.default')]);
        }
    ...
    public function getTranslatedAttributeMeta($attribute, $locale = null, $fallback = true)
    {
        ...
        if (!in_array($attribute, $this->getTranslatableAttributes())) {
            return [$this->getAttribute($attribute), config('hymer.multilingual.default'), false];
        }
        ...
        $default = config('hymer.multilingual.default');
    ...
    public function setAttributeTranslations($attribute, array $translations, $save = false)
    {
        ...
        $default = config('hymer.multilingual.default', 'en');
        $locales = config('hymer.multilingual.locales', [$default]);
    ...
    public function prepareTranslations($request)
    {
        ...
        foreach ($fields as $field) {
            ...
            $request->merge([$field => $trans[config('hymer.multilingual.default', 'en')]]);
    ...
    public function prepareTranslationsFromArray($field, &$requestData)
    {
        ...
        // Set the default local value
        $requestData['display_name'] = $trans[config('hymer.multilingual.default', 'en')];
