...
class InstallCommand extends Command
{
     ...
     * @var string
     */
    protected $description = 'Install the Hymer Admin package';
    ...
    {
        $this->info('Publishing the Hymer assets, database, and config files');

        // Publish only relevant resources on install
        $tags = ['seeders'];

        $this->call('vendor:publish', ['--provider' => HymerServiceProvider::class, '--tag' => $tags]);

        $this->info('Migrating the database tables into your application');
        $this->call('migrate', ['--force' => $this->option('force')]);

        $this->info('Attempting to set Hymer User model as parent to App\User');
        ...
        if (false === strpos($routes_contents, 'Hymer::routes()')) {
            $filesystem->append(
                base_path('routes/web.php'),
                PHP_EOL.PHP_EOL."Route::group(['prefix' => 'admin'], function () {".PHP_EOL."    Hymer::routes();".PHP_EOL."});".PHP_EOL
            );
        }
        ...
        if ($this->option('with-dummy')) {
            ...
            $this->call('vendor:publish', ['--provider' => HymerDummyServiceProvider::class, '--tag' => $tags]);
        } else {
            $this->call('vendor:publish', ['--provider' => HymerServiceProvider::class, '--tag' => ['config', 'hymer_avatar']]);
        }
        ...
        $this->call('db:seed', ['--class' => 'HymerDatabaseSeeder', '--force' => $this->option('force')]);
        ...
        if ($this->option('with-dummy')) {
            ...
            $this->call('db:seed', ['--class' => 'HymerDummyDatabaseSeeder', '--force' => $this->option('force')]);
        }

        $this->info('Adding the storage symlink to your public folder');
        $this->call('storage:link');

        $this->info('Successfully installed Hymer! Enjoy');
    }
