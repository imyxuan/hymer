...
class DataType extends Model
{
    ...
    {
        return $this->hasMany(Hymer::modelClass('DataRow'))->orderBy('order');
    }
    ...
    {
        return $this->hasMany(Hymer::modelClass('DataRow'))->orderBy('order', 'DESC')->first();
    }
        ...
        try {
            ...
            if ($this->fill($requestData)->save()) {
                ...
                foreach ($fields as $field) {
                    ...
                    if (!$dataRow->save()) {
                        throw new \Exception(__('hymer::database.field_safe_failed', ['field' => $field]));
                    }
                ...
                if ($this->generate_permissions) {
                    Hymer::model('Permission')->generateFor($this->name);
                }
