...
class Menu extends Model
{
    ...
    {
        return $this->hasMany(Hymer::modelClass('MenuItem'));
    }
    ...
    public static function display($menuName, $type = null, array $options = [])
    {
        ...
        $menu = \Cache::remember('hymer_menu_'.$menuName, \Carbon\Carbon::now()->addDays(30), function () use ($menuName) {
        ...
        if ($type == 'admin') {
            $type = 'hymer::menu.'.$type;
        } else {
            if (is_null($type)) {
                $type = 'hymer::menu.default';
            } elseif ($type == 'bootstrap' && !view()->exists($type)) {
                $type = 'hymer::menu.bootstrap';
            }
        }
    ...
    {
        \Cache::forget('hymer_menu_'.$this->name);
    }
    ...
    protected static function processItems($items)
    {
        ...
        $items = $items->transform(function ($item) {
            ...
            if (($item->href == url('') || $item->href == route('hymer.dashboard')) && $item->children->count() > 0) {
                // Exclude sub-menus
                $item->active = false;
            } elseif ($item->href == route('hymer.dashboard') && url()->current() != route('hymer.dashboard')) {
                // Exclude dashboard
                $item->active = false;
            }
