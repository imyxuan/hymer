...
class CompassTest extends TestCase
{
    ...
    {
        $this->app['config']->set('hymer.compass_in_production', true);
    }
    ...
    public function testCantAccessCompass()
    {
        ...
        $response = $this->call('GET', route('hymer.compass.index'));
        $this->assertEquals(403, $response->status());
    }

    public function testCanAccessCompass()
    {
        // Can access compass because we set hymer.compass_in_production configuration to true
        $this->enableCompass();
    ...
    public function testCanSeeLaravelLog()
    {
        ...
        $this->visit(route('hymer.compass.index').'?log='.base64_encode('laravel.log'))
             ->see($info);
    }
    ...
    public function testCanExecuteCommand()
    {
        ...
        $response = $this->post(route('hymer.compass.index'), [
            'command' => 'make:model',
            'args'    => 'TestModel',
        ]);
    ...
    public function testCanDeleteLaravelLog()
    {
        ...
        $response = $this->call('GET', route('hymer.compass.index').'?del='.base64_encode('laravel.log'));
